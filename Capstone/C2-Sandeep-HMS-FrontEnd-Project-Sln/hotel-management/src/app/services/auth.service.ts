...
export class AuthService {
  private dbUrl = 'http://localhost:8080/api/v1/register'; // URL for the database
  ...
  login(email: string, password: string): Observable<any> {
    ...
    return this.getUsers().pipe(
      map((users) => {
        ...
        this.attemptsLeft = 3; // Reset attempts counter on success
        this.setLoggedInState(true); // Persist login state
        return user;
      }),
      catchError((error) => {
        throw error;
  ...
  // Logout
  logout(): void {
    this.setLoggedInState(false); // Clear login state
  }
  ...
  private lockAccount(): void {
    this.lockTimeout = setTimeout(() => {
      this.lockTimeout = null; // Unlock account
